	...
	public void buttonMatrix(int width, int length) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < length; y++) {
				...
				grid[x][y].setPreferredSize(new Dimension(25, 25));
				...
				grid[x][y].addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						String s = (String) JOptionPane.showInputDialog(frame, "Selezionare l'opzione",
								"Plan the battle", JOptionPane.PLAIN_MESSAGE, null, possibilities, "water");
		...
		try {
			...
			for (int x = 0; x < rows; x++)
				for (int y = 0; y < cols; y++) {
					...
					writer.write("(cell (x " + x + ") (y " + y + ") (content " + matrix[x][y].getContentSimple()
							+ ") (status none))\n");
			...
			for (int x = 0; x < rows; x++)
				for (int y = 0; y < cols; y++) {
					...
					if (matrix[x][y].isSub())
						writer.write("(boat-hor (name sub" + ++counter + ") (x " + x + ") (ys " + y
								+ ") (size 1) (status safe))\n");
